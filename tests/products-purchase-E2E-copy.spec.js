...
test('test', async ({ page }) => {
    ...
    for (let counter = 0; counter <TotalProductIDs ; counter++) {
        ...
        await expect(page.locator(`[id='product-${productIdCounter}'] > .cart_description`)).toBeVisible();
        await expect(page.locator(`[id='product-${productIdCounter}'] > .cart_price`)).toBeVisible();
        await expect(page.locator(`#product-${productIdCounter} > .cart_quantity > .disabled`)).toBeVisible();
        await expect(page.locator(`#product-${productIdCounter} > .cart_total > .cart_total_price`)).toBeVisible();
    }
